  ...
  >
    ...
    >
        ...
        <template slot-scope="props">
          <el-form label-position="left" inline class="demo-table-expand">
            ...
            <el-form-item label="职位审核状态" style="width: 100%">
              ...
              >
                {{ props.row.status+5 | statusFilter }}
              </el-tag>
            </el-form-item>
      ...
      <el-table-column label="Operation" align="center">
        <template slot-scope="scope">
          <el-button v-if="scope.row.status === 0 || scope.row.status >= 2" size="small" type="danger" icon="el-icon-delete" circle @click="changePositionStatus(scope.row.id, scope.$index, 1)" />
          <el-button v-if="scope.row.status === 0 || scope.row.status === 1" size="small" type="success" icon="el-icon-check" circle @click="changePositionStatus(scope.row.id, scope.$index, 2)" />
...
export default {
  ...
  methods: {
    changePositionStatus(positionId, index, status) {
      changePositionStatus(positionId, { status }).then((response) => {
        ...
        if (response.code === 200) {
          ...
          this.data.items[index].status = status === 1 ? 1 : 2
        }
      })
    }
  }
